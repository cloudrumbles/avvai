...
import { themes, vars } from './theme.css';
import { getThemeInfo, tamilFonts, thinaiList, type ThemeKey } from './themeData';
  ...
  return (
    <div class={s.tabsRoot}>
      <div class={s.tabsList}>
        <button
          class={s.tabsTrigger({ active: activeTab() === 'account' })}
          onClick={() => setActiveTab('account')}
        >Account</button>
        <button
          class={s.tabsTrigger({ active: activeTab() === 'password' })}
          onClick={() => setActiveTab('password')}
        >Password</button>
        <button
          class={s.tabsTrigger({ active: activeTab() === 'settings' })}
      ...
      <Show when={activeTab() === 'account'}>
        <div class={s.tabsContent}>
          ...
          <div class={s.btnGroup} style={{ "margin-top": "1rem" }}>
            <button class={s.btn({ color: 'primary' })}>Save changes</button>
          </div>
        </div>
      </Show>
      <Show when={activeTab() === 'password'}>
        <div class={s.tabsContent}>
          ...
          <button class={s.btn({ color: 'secondary' })}>Update password</button>
        </div>
      </Show>
...
function App() {
  ...
  const [currentView, setView] = createSignal<'themes' | 'fonts' | 'mockup'>('themes');
  const [headingFont, setHeadingFont] = createSignal('Noto Serif Tamil');
  const [bodyFont, setBodyFont] = createSignal('Mukta Malar');

  const themeKey = () => `${currentThinnai()}-${currentMode()}` as ThemeKey;

  createEffect(() => {
    document.body.className = themes[themeKey()];
  });

  const currentTheme = () => getThemeInfo(themeKey());
    ...
    <div class={s.layout}>
      <aside class={s.sidebar}>
        ...
        <ul class={s.themeList}>
          <li>
            <div
              class={s.themeRow({ active: currentView() === 'themes' })}
          ...
          <li>
            <div
              class={s.themeRow({ active: currentView() === 'fonts' })}
          ...
          <li>
            <div
              class={s.themeRow({ active: currentView() === 'mockup' })}
              onClick={() => setView('mockup')}
            >
              <button class={s.themeSelect}>App Mockup</button>
            </div>
          </li>
        </ul>
        ...
        <Show when={currentView() === 'themes'}>
          ...
          <ul class={s.themeList}>
            <For each={thinaiList}>{(t) => (
              <li>
                <div
                  class={s.themeRow({ active: currentThinnai() === t.id })}
                ...
                >
                  <button class={s.themeSelect}>
                    ...
                    <span>
                      {t.name}
                      <span class={s.themeMeta}>{t.emotion}</span>
                    </span>
                  </button>
                  <div class={s.modeToggle}>
                    <button
                      class={s.modeBtn({ active: currentThinnai() === t.id && currentMode() === 'light' })}
                    ...
                    <button
                      class={s.modeBtn({ active: currentThinnai() === t.id && currentMode() === 'dark' })}
      ...
      <main class={s.main}>
          ...
          <div class={s.themeInfo}>
            <div class={s.themeInfoTitle}>
              {currentTheme().name} {currentTheme().mode === 'light' ? 'Light' : 'Dark'} ({currentTheme().tamil})
            </div>
            <div class={s.themeInfoMeta}>
              {currentTheme().landscape} — {currentTheme().emotion}
            </div>
          </div>
          ...
          <section class={s.section} id="colors">
            ...
            <div class={s.swatches}>
              <For each={Object.keys(currentTheme().palette) as (keyof typeof currentTheme.palette)[]}>{(role) => {
                const value = () => currentTheme().palette[role];
                const contrast = () => currentTheme().getContrast(role);
                ...
                return (
                  <div class={s.swatch}>
                    <div class={s.swatchColor} style={{
                      ...
                      <span class={s.swatchLabel}>{value()}</span>
                    </div>
            ...
            <div class={s.componentsGrid}>
              <div class={s.componentSection}>
                ...
                <div class={s.btnGroup}>
                  <button class={s.btn({ color: 'primary' })}>Primary</button>
                  <button class={s.btn({ color: 'secondary' })}>Secondary</button>
                  <button class={s.btn({ color: 'outline' })}>Outline</button>
                  <button class={s.btn({ color: 'ghost' })}>Ghost</button>
                </div>
                <div class={s.btnGroup}>
                  <button class={s.btn({ color: 'primary' })} disabled>Disabled</button>
                  <button class={s.btn({ color: 'secondary' })} disabled>Disabled</button>
                </div>
              </div>
              ...
              <div class={s.componentSection}>
                ...
                <div class={s.badgeGroup} style={{ "margin-bottom": "1rem" }}>
                  <span class={s.badge({ color: 'primary' })}>Primary</span>
                  <span class={s.badge({ color: 'secondary' })}>Secondary</span>
                  <span class={s.badge({ color: 'accent' })}>Accent</span>
                  <span class={s.badge({ color: 'muted' })}>Muted</span>
                </div>
        ...
        <Show when={currentView() === 'mockup'}>
          <div class={s.themeInfo}>
            <div class={s.themeInfoTitle}>App Mockup</div>
            <div class={s.themeInfoMeta}>Tamil learning app prototype (SolidJS)</div>
          </div>
